...
# Ele apenas cuida da parte visual das linhas.
# ============================================================
...
@export var z_index_linha_preview: int = 6
@export var z_index_linha_preview_sombra: int = 5
...
@export var z_index_linha_final: int = 2
@export var z_index_linha_final_sombra: int = 1
...
func criar_linha(posicao_inicial: Vector2, posicao_final_inicial: Vector2) -> void:
	...
	linha_sombra_atual.name = "LinhaPreviewSombra"
	...
	linha_sombra_atual.z_index = z_index_linha_preview_sombra
	linha_sombra_atual.add_point(posicao_inicial)
	linha_sombra_atual.add_point(posicao_final_inicial)
	camada_linhas.add_child(linha_sombra_atual)

	linha_atual = Line2D.new()
	linha_atual.name = "LinhaPreview"
	...
	linha_atual.z_index = z_index_linha_preview
	linha_atual.add_point(posicao_inicial)
	linha_atual.add_point(posicao_final_inicial)
	camada_linhas.add_child(linha_atual)


func atualizar_linha(posicao_final: Vector2) -> void:
	if linha_atual:
		linha_atual.set_point_position(1, posicao_final)

	if linha_sombra_atual:
		linha_sombra_atual.set_point_position(1, posicao_final)
